...
sub xml_dataobj
{
	...
	my %relator_map = (
		...
		# The following are not in the EPrints list, but it is a part of LOC relators, so good candidate to add
		"http://www.loc.gov/loc.terms/relators/DTM" => "DataManager",
		"http://www.loc.gov/loc.terms/relators/HIS" => "HostingInstitution",
	...
	# allow local customisations to addi/re-map items types
	%type_map = $plugin->_add_local_config( "type_map", \%type_map );
	...
	# allow local customisations to add items types, or update entries in map above
	%type_map_uri = $plugin->_add_local_config( "type_map_uri", \%type_map_uri );
	...
	#resourceType
	$topcontent = $session->make_element(
		"oaire:resourceType",
		"uri" => "$mapped_type_URI",
		"resourceTypeGeneral" => "$mapped_resourceTypeGeneral"
	);
...
sub _add_local_config
{
	my( $plugin, $conf_name, $map ) = @_;
